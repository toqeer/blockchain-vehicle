...
export class NewOrders extends Component {
  ...
  componentDidMount() {
    let params={ orderStatusNot: "DELIVERED" }
  ...
  handleSubmit = orderId => {
    const { orderStatus, vIn } = this.state;
    const data = {
      orderId,
      vIn,
      orderStatus
    };
    this.props.updateVehicleOrderStatus(data,this.handleClose);
  };
  
  render() {
    const { orders, loading } = this.props;
    const { selectedOrderId,orderStatus, vIn } = this.state;
    let selectedOrder = {};
